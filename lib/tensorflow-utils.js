...
module.exports = {
    ...
    InitTensorFlow: function(rawData, settings) {
        ...
        tfX = tf.tensor3d(formatData.x);
        tfY = tf.tensor3d(formatData.y);
        formatData.dispose;
...
function createNN(){
    try{
        ...
        layers: [
            ...
            tf.layers.dense({units: 150, activation: 'relu', inputShape: [INLINES_NUM,INLINE_SIZE]}),
            tf.layers.dense({units: 150, activation: 'relu'}),
            tf.layers.conv1d( { filters : 150, kernelSize: INLINES_NUM - OUTLINES_NUM*OUTLINE_SIZE + 1} ),
            tf.layers.dense({units: 150, activation: 'relu'}),
