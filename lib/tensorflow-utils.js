...
module.exports = {
    ...
    InitTensorFlow: function(rawData, settings) {
        ...
        for(var i=0;i<rawData.length;i++){
            ...
            var yRows = [];
            var result = rawData[i].resultList[0];
            ...
            while(yRows.length < OUTLINES_NUM) yRows.push(  new Array(ALLTOKENS_LENGTH).fill(0)  );
    ...
    predict: function(rawData,settings){
        ...
        var result = [];
        var resRow = [];
        predictions.forEach(function(ele,idx){
            if(Math.round(ele) == 1)
                resRow.push( {val:ele,label:settings.allTokens[idx % ALLTOKENS_LENGTH]} );
        });
        result.push(resRow);
...
function createNN(){
    try{
        ...
        layers: [
            ...
            tf.layers.conv1d( { filters : 40, kernelSize: INLINES_NUM-OUTLINES_NUM+1} ),
