...
const INLINES_NUM = 12;
const OUTLINES_NUM = 1;
    ...
    InitTensorFlow: function(rawData, settings) {
        ...
        for(var i=0;i<rawData.length;i++){
            ...
            rawData[i].resultList.forEach(function(result){
                ...
                if(tokens.length > OUTLINE_SIZE) tokens = tokens.splice(0,OUTLINE_SIZE);
                while(tokens.length < OUTLINE_SIZE)tokens.push(0);
                yRows.push(tokens);
            });  
            if(yRows.length > OUTLINES_NUM) yRows = yRows.splice(0,OUTLINES_NUM);
            while(yRows.length < OUTLINES_NUM) yRows.push(  new Array(OUTLINE_SIZE).fill(0)  );
...
function createNN(){
    try{
        ...
        layers: [
            ...
            tf.layers.dense({units: 20, activation: 'relu', inputShape: [INLINES_NUM,INLINE_SIZE]}),
            tf.layers.dense({units: 20, activation: 'relu'}),
            tf.layers.conv1d( { filters : 10, kernelSize: INLINES_NUM-OUTLINES_NUM+1} ),
            tf.layers.dense({units: 20, activation: 'relu'}),
            tf.layers.dense({units: OUTLINE_SIZE, activation:'softmax'})
        ]
        });
        model.compile(
...
async function Training(){
    ...
    for (let i = 0; i < TRAIN_BATCHES; i++) {
        const history = await neuralNetwork.fit(
            tfX,
            tfY,
            {batchSize: BATCH_SIZE, epochs: EPOCHS, validationSplit:0.1, shuffle:true}
        );
